...
find "$ROOT/src/main/java" -name '*.java' -print0 | xargs -0 javac --release 21 -encoding UTF-8 -d "$ROOT/build/classes"
jar --create --file "$ROOT/build/streamflix-desktop.jar" --main-class dev.streamflix.desktop.App -C "$ROOT/build/classes" .
find "$ROOT/src/test/java" -name '*.java' -print0 | xargs -0 javac --release 21 -encoding UTF-8 -cp "$ROOT/build/classes" -d "$ROOT/build/test-classes"
